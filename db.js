...
console.log('SQLite cwd =', process.cwd());
...
const init = async () => {
  ...
  if (!voff) {
    await db.runAsync(`INSERT INTO visitor_offset (id, offset_count) VALUES (1, 0)`);
  }
  ...
  if (!pc || pc.c === 0) {
    ...
    for (const p of projects) {
      await db.runAsync(
        `INSERT INTO projects (title,description,tech_stack,status,category,github_url,live_url,featured,sort_order,start_date,end_date)
         VALUES (?,?,?,?,?,?,?,?,?,?,?)`,
        p
      );
    }
  ...
  if (!ec || ec.c === 0) {
    ...
    for (const e of exp) {
      await db.runAsync(
        `INSERT INTO experience (company,position,description,start_date,end_date,is_current,sort_order)
         VALUES (?,?,?,?,?,?,?)`,
        e
      );
    }
